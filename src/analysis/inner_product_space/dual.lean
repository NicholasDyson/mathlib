...
the map `λ y, ⟪x, y⟫`.  Auxiliary construction for `to_dual` and `to_dual`.
--/
def to_dual_aux (x : E) : normed_space.dual 𝕜 E :=
...
{ to_fun := to_dual_aux 𝕜 E,
  map_add' := λ x y, by { ext z, simp [to_dual_aux, inner_add_left] },
  map_smul' := λ c y, by { ext z, simp [to_dual_aux, inner_smul_left] },
  norm_map' := λ x, begin
    ...
    { cases eq_or_lt_of_le (norm_nonneg x) with h h,
      ...
      { refine (mul_le_mul_right h).mp _,
        ...
        ... ≤ abs ⟪x, x⟫ : re_le_abs _
        ... = ∥to_dual_aux 𝕜 E x x∥ : by simp [to_dual_aux, norm_eq_abs]
        ... ≤ ∥to_dual_aux 𝕜 E x∥ * ∥x∥ : le_op_norm (to_dual_aux 𝕜 E x) x } }
  end }
...
-/
def to_dual [complete_space E] : E ≃ₗᵢ*[𝕜] normed_space.dual 𝕜 E :=
...
@[simp] lemma to_dual_apply [complete_space E] {x y : E} : to_dual 𝕜 E x y = ⟪x, y⟫ := rfl
